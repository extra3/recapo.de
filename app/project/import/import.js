require(['jquery', 'jquery-form'], function($){
  ...
  var progressBarFormExample = '#example form';
  $(progressBarFormExample).ajaxForm({
    beforeSend: function() {
      $(progressBarFormExample).find(progressBar).empty().css('width', '0%').addClass('progress-bar-success').removeClass('progress-bar-danger');
      $(progressBarFormExample).find(':input').attr('disabled', true);
    },
    uploadProgress: function(event, position, total, percentComplete) {
      var percentVal = percentComplete + '%';
      $(progressBarFormExample).find(progressBar).html(percentVal).css('width', percentVal);
    },
    success: function() {
      var percentVal = '100%';
      $(progressBarFormExample).find(progressBar).html(percentVal).css('width', percentVal);
    ...
    complete: function(xhr) {
      if(xhr.status == 200) {
        $(progressBarFormExample).find(progressBar).html(xhr.responseText).addClass('progress-bar-success').removeClass('progress-bar-danger');
      } else {
        $(progressBarFormExample).find(':input').attr('disabled', false);
        $(progressBarFormExample).find(progressBar).html(xhr.responseText).css('width', '100%').addClass('progress-bar-danger').removeClass('progress-bar-success');
      }
    }
  });
