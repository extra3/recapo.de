...
$_callback = function ($pFile, $pFileName) use ($_app, $_params, $login) {
    ...
    switch ($name) {
      ...
      $data = utf8_encode($data);
      $data = explode("\n", $data);
      $data = array_map(function ($p) { return str_getcsv($p, ';'); }, $data);
...
$data = $pFile;
$stats = array('link' => 0, 'id' => 0, 'i' => 0, 'newi' => 0, 'c' => 0, 'newc' => 0, 'img' => 0);
  ...
  $_parentLevel = 0;
    ...
    for ($i = 0; $i<$rows; $i++) {
        ...
        array_push($parentStack, $_parentID);
                ...
                if (preg_match('/'.$sectionsRegEx.'(((link|id|i|newi|c|newc|img|)\((.+)\))|(.+))/i', $data[$i][$j], $matches) != 1) {
        ...
        if (empty($matches[4])) {
          ...
          case 'img':
            ...
            $_ID = \Recapo\Model\Image::issetImageByName($projectID, $_name);
            if ($_ID == null) {
              // otherwise create a dummy image
              $_ID = \Recapo\Model\Image::insertImageReturnID($login->info['userID'], $projectID, 'dummy', 'png', 'dummy:'.$_name);
            }
            $stats['img']++;
            break;
...
if (isset($_FILES['file']) && trim($_FILES['file']['name'])) {
...
} elseif (isset($_POST['importExample'])) {
  $files = array(
    '../web/example/2014-08-18_-_dlrg_-_container.csv',
    '../web/example/2014-08-18_-_dlrg_-_task.csv',
    '../web/example/2014-08-18_-_dlrg_-_section.csv',
    '../web/example/2014-08-18_-_dlrg_-_item.csv',
    '../web/example/2014-08-18_-_dlrg_-_ia-extended.csv',
  );
  ...
  //backup parameter
  $backupNameParameter = $_params['NAME'];

  //call function for every file
  for ($i = 0; $i<5; $i++) {
    $file = file_get_contents($files[$i]);
    $_params['NAME'] = $names[$i];
    importFile($file, $_params);
  }

  //restore original parameter
  $_params['NAME'] = $backupNameParameter;


  print 'Beispielprojekt importiert';
} else {
...
/*
 * quick & dirty: copy from the anonymous function above
 * $_callback = function ($pFile, $pFileName) use ($_app, $_params, $login)
 */
function importFile($pFile, $_params) {
  $projectID = $_params['ID'];
  $name = $_params['NAME'];
